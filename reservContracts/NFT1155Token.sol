...
// https://www.youtube.com/watch?v=b7LACuU_X94
// https://docs.opensea.io/docs/opensea-erc1155-tutorial

contract NFT1155Token is
    IERC1155,
    Ownable // about abstract https://ethereum.stackexchange.com/questions/83267/contract-should-be-marked-as-abstract
{
    ...
    function mint(
        ...
        string calldata _uri // чеканка токенов, доступна всем, аргум: количество NFT и строка URI, URL, CID
    ...
    function mintBatch(
        // чеканка пачкой сущетвующего токенов на адреса по id создателя этого токена.
        uint256 _id, // id NFT (создателя, тд уже созданного токена) / "1"
        address[] calldata _to, // адреса кому чеканить токены / ["0x5B...", "0x4...", "0x78..."]
        uint256[] calldata _amounts // количество токенов по указанному _id /  ["10", "10", "10"]
    ...
    function transerFrom(
        // 1 метод вызывает списывающая токены сторона(_to) по _id токена в указанном количестве _value
        // 2 если нет approve у _to то transerFrom в стандартной реализации from -> to 
        address _from,
        address _to,
        uint256 _id,
        uint256 _value
    ...
    function batchTransferFrom(
        // аналогичный transerFrom, только отправка пачкой, метод вызывает списывающая сторона _to
        address _from,
        address _to,
        uint256[] calldata _ids, // ["1", "2", ...]
        uint256[] calldata _values // ["10", "5", ...]
    ) external override {
        require(_to != address(0x0), "_to must be not zero");
        require(
    ...
    function balanceOfBatch(
        // баланс по масиву _owners и массиву _ids
        address[] memory _owners,
        uint256[] memory _ids // ["0x5B3...", "0xAb8..."] ["1", "2"]
    ...
    function approve(
        // вызывает owner токена
        // разрешение на списание
        address _spender,
        uint256 _id,
        uint256 _currentValue, // текущее уже разрешенное value на списание у spender по id токена
        uint256 _value // новое значение на списание по _id
    ) external override {
    ...
    function checkAllowance(
        // есть ли доступ кошелька _spender к кошельку _owner к какому либо токену _owner по id этого токена
        address _owner,
        address _spender,
        uint256 _id
